...
class TagihanController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            //      ->whereMonth('tanggal_jatuh_tempo', $bulan)
            //      ->whereYeard('tangga_jatuh_tempo', $tahun)
            // }
            ...
            if ($tagihans->isEmpty()) {
                ...
                foreach ($iuran as $item) {
                    Tagihan::create([
                        'user_id' => $user->id,
                        'iuran_id' => $item->id,
                        'tanggal_jatuh_tempo' => Carbon::create($tahun, $bulan, 20),
                        'status' => 'belum_bayar'
                    ]);
                }
                $tagihans = Tagihan::where('user_id', $user->id)
            ...
            // Memeriksa dan memperbarui status tagihan dari Midtrans
    ...
    public function bayar(Tagihan $tagihan)
    {
        ...
        // Membuat order ID yang unik
        $orderId = 'TAGIHAN-' . $tagihan->id . '-' . time();

        // Menyiapkan parameter untuk request SnapToken
        $params = [
            'transaction_details' => [
                'order_id' => $orderId,
                'gross_amount' => $tagihan->iuran->jumlah, // Mengambil jumlah dari relasi iuran
            ],
            'customer_details' => [
        ...
        try {
            // Mendapatkan Snap Token untuk melakukan pembayaran
        ...
        // Menyimpan status tagihan sementara dengan status 'belum_bayar'
        Tagihan::updateOrCreate(
            ['id' => $tagihan->id],
            ['order_id' => $orderId, 'snap_token' => $snapToken, 'status' => 'belum_bayar']
        );

        // Mengembalikan view pembayaran dengan Snap Token
    ...
    {
        $bulan = $request->input('bulan', now()->month);  // bulan saat ini
        $tahun = $request->input('tahun', now()->year);   // tahun saat ini


        // Ambil tagihan yang sudah dibayar pada bulan dan tahun tertentu, urutkan berdasarkan tanggal jatuh tempo terbaru
        $tagihansSudahBayar = Tagihan::with('user', 'iuran')
            ->where('status', 'sudah_bayar')
            ->whereMonth('tanggal_jatuh_tempo', $bulan)
            ->whereYear('tanggal_jatuh_tempo', $tahun)
            ->orderBy('tanggal_jatuh_tempo', 'desc')  // Urutkan berdasarkan tanggal jatuh tempo terbaru
            ->paginate(5, ['*'], 'sudah_bayar_page');

        // Ambil tagihan yang sudah dibayar pada bulan dan tahun lain, urutkan berdasarkan tanggal jatuh tempo terbaru
        ...
        // Update status dan tanggal bayar berdasarkan informasi dari Midtrans
        $this->updateStatusTanggalBayar($tagihansSudahBayar);
        $this->updateStatusTanggalBayar($tagihansBulanLain);

        return view('tagihan.laporan', compact('tagihansSudahBayar', 'tagihansBulanLain', 'bulan', 'tahun'));
    }
    ...
    public function triggerNotifikasiWABelumBayar()
    {
        ...
        foreach ($users as $user) {
            ...
            curl_setopt_array($curl, [
                ...
                CURLOPT_HTTPHEADER => [
                    'Authorization: Y7ZvCw6UXWTLCsKAj8nX' // Token Fonnte
                ],
            ]);
