<x-app-layout>
    <div class="max-w-7xl mx-auto p-6 bg-white shadow-lg rounded-xl">
        ...
        <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <form method="GET" class="flex items-center gap-2 bg-gray-50 border border-gray-200 px-4 py-2 rounded-md shadow-sm">
                <label for="bulan" class="text-sm font-medium text-gray-700">Bulan:</label>
                <input type="number" name="bulan" value="{{ $bulan }}" min="1" max="12" class="w-20 border border-gray-300 rounded-md text-sm p-1 focus:ring-indigo-500 focus:border-indigo-500" />

                <label for="tahun" class="text-sm font-medium text-gray-700">Tahun:</label>
                <input type="number" name="tahun" value="{{ $tahun }}" class="w-24 border border-gray-300 rounded-md text-sm p-1 focus:ring-indigo-500 focus:border-indigo-500" />

                <button type="submit" class="ml-2 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    Tampilkan
                </button>
            </form>
        </div>
        ...
        <div class="overflow-x-auto bg-white shadow-md rounded-lg mb-6">
            <h3 class="text-xl font-semibold my-3 px-3">List Warga yang Sudah Bayar Bulan Ini ({{ $bulan }}/{{ $tahun }})</h3>
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-800">Rp {{ number_format($tagihan->iuran->jumlah, 0, ',', '.') }}</td>
                    </tr>
                    @empty
                    <tr>
        ...
        <div class="overflow-x-auto bg-white shadow-md rounded-lg">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3 text-sm text-gray-800">{{ $tagihan->user->name }}</td>
                        <td class="px-4 py-3 text-sm text-gray-800">{{ \Carbon\Carbon::parse($tagihan->tanggal_jatuh_tempo)->format('m/Y') }}</td>
                        <td class="px-4 py-3 text-sm text-gray-800">Rp {{ number_format($tagihan->iuran->jumlah, 0, ',', '.') }}</td>
                    </tr>
                    @empty
                    <tr>
